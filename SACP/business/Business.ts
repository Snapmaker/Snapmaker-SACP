...
import MovementInstruction, {
  MoveDirection,
} from "./models/MovementInstruction";
...
export default class Business extends Dispatcher {
  ...
  constructor(type: string, socket: any) {
    ...
    this.setHandler(0xb0, 0x00, async (data) => {
      ...
      const outputStream = fs.createWriteStream(
        path.join(DataStorage.tmpDir, filename)
      ); // need to rename this filename
          ...
          await this.send(0xb0, 0x01, PeerId.SCREEN, buffer).then((res) => {
            if (res.response.result === 0) {
              const {
                ...
                result: md5HexStr1,
              } = readString(res.response.data, 0);
              const chunkIndex1 = readUint16(res.response.data, _nextOffset);
              const len = readUint16(res.response.data, _nextOffset + 2);
              receivedLength += len;
              const chunk = res.response.data.slice(
                _nextOffset + 2 + 2,
                _nextOffset + 2 + 2 + len
              );
              this.log.debug(
                `received chunk: ${chunkIndex} success, ${md5HexStr1}, ${chunkIndex1}, ${len}`
              );
      ...
      this.send(
        ...
        0x02,
        PeerId.SCREEN,
        Buffer.concat([resultBuffer, filenameBuffer, md5Buffer])
      );
    });
  }
  ...
  getPhoto(index: number = 0) {
    return this.send(0xb0, 0x05, PeerId.SCREEN, Buffer.alloc(1, index)).then(
      ({ response, packet }) => {
        const data = {
          filename: "",
          md5Str: "",
  ...
  getPosition(params) {
    const { headType, modelId } = params;
    const buffer = Buffer.alloc(2, 0);
    writeUint8(buffer, 0, modelId);
    writeUint8(buffer, 1, headType);
    console.log("response", params, buffer);
    return this.send(0xb0, 0x91, PeerId.SCREEN, buffer).then(({ response }) => {
      console.log("response", response, response.data);
      if (response.result === 0) {
        const data = response.data;
        const x = readFloat(data, 0);
        const y = readFloat(data, 4);
        const z = readFloat(data, 8);
        return { response, x, y, z };
      }
    });
  }
  setPosition(params) {
    const { headType, modelId, positionX, positionY, positionZ } = params;
    console.log("params", params);
    const buffer = Buffer.alloc(14, 0);
    let nextOffset = 0;
    writeUint8(buffer, 0, modelId);
    nextOffset = writeUint8(buffer, 1, headType);
    nextOffset = writeFloat(buffer, nextOffset, positionX);
    nextOffset = writeFloat(buffer, nextOffset, positionY);
    nextOffset = writeFloat(buffer, nextOffset, positionZ);
    return this.send(0xb0, 0x90, PeerId.SCREEN, buffer).then(({ response }) => {
      if (response.result === 0) {
        return { response };
      }
    });
  ...
  setMatrix(
    toolHeadType: ToolHeadType = ToolHeadType.LASER1600mW,
    matrix: CalibrationInfo
  ) {
    ...
    return this.send(
      ...
      0x07,
      PeerId.SCREEN,
      Buffer.concat([Buffer.alloc(1, toolHeadType), calibrationInfo.toBuffer()])
    );
  }
  ...
  startScreenPrint({ headType = HeadType.PRINTING, filename = "", hash = "" }) {
    ...
    return this.send(
      ...
      0x08,
      PeerId.SCREEN,
      Buffer.concat([Buffer.alloc(1, headType), filenameBuffer, hashBuffer])
    );
  }
  ...
  getLaserMaterialThickness({ token = "", x = 0, y = 0, feedRate = 0 }) {
    ...
    return this.send(0xb0, 0x09, PeerId.SCREEN, Buffer.concat([buffer])).then(
      ({ response, packet }) => {
        let thickness = 0;
        if (response.result === 0) {
  ...
  moveAbsolutely(
    movementInstructions: Array<MovementInstruction>,
    speed: number = 0
  ) {
    ...
    movementInstructions.forEach((item) => {
      const m = new MovementInstruction(
        item.direction,
        item.distance
      ).toBuffer();
      buffer = Buffer.concat([buffer, m]);
    ...
    return this.send(
      ...
      0x34,
      PeerId.CONTROLLER,
      Buffer.concat([buffer, speedBuffer])
    );
  }
  ...
  getLaserToolHeadInfo(key: number) {
    ...
    return this.send(0x12, 0x01, PeerId.CONTROLLER, buffer).then(
      ({ response }) => {
  ...
  subscribeHeartbeat({ interval = 1000 }, callback: ResponseCallback) {
    return this.subscribe(0x01, 0xa0, interval, callback).then(
      ({ response, packet }) => {
  ...
  unsubscribeHeartbeat(callback: ResponseCallback) {
    return this.unsubscribe(0x01, 0xa0, callback).then(
      ({ response, packet }) => {
        return { code: response.result, packet, data: {} };
      }
    );
  ...
  getModuleInfo() {
    return this.send(0x01, 0x20, PeerId.CONTROLLER, Buffer.alloc(0)).then(
      ({ response, packet }) => {
        const moduleInfos = ModuleInfo.parseArray(
          response.data
        ) as ModuleInfo[];
        return {
          code: response.result,
          packet,
          data: moduleInfos as ModuleInfo[],
        };
      }
    );
  }
  ...
  getMachineInfo() {
    return this.send(0x01, 0x21, PeerId.CONTROLLER, Buffer.alloc(0)).then(
      ({ response, packet }) => {
        const machineInfo = new MachineInfo().fromBuffer(response.data);
        return {
          code: response.result,
          packet,
          data: machineInfo as MachineInfo,
        };
      }
    );
  }
  ...
  getMachineSize() {
    return this.send(0x01, 0x22, PeerId.CONTROLLER, Buffer.alloc(0)).then(
      ({ response, packet }) => {
  ...
  getCurrentCoordinateInfo() {
    return this.send(0x01, 0x30, PeerId.CONTROLLER, Buffer.alloc(0)).then(
      ({ response, packet }) => {
        const coordinateSystemInfo = new CoordinateSystemInfo().fromBuffer(
          response.data
        );
        return {
          response,
          packet,
          data: { coordinateSystemInfo },
  ...
  movementInstruction(direction: MoveDirection, distance: number) {
    ...
    return this.send(0x01, 0x34, PeerId.CONTROLLER, info.toBuffer()).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  startPrint(md5: string, gcodeName: string, headType: number) {
    ...
    return this.send(0xac, 0x03, PeerId.CONTROLLER, info.toBuffer()).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  stopPrint() {
    return this.send(0xac, 0x06, PeerId.CONTROLLER, Buffer.alloc(0)).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  pausePrint() {
    return this.send(0xac, 0x04, PeerId.CONTROLLER, Buffer.alloc(0)).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  resumePrint() {
    return this.send(0xac, 0x05, PeerId.CONTROLLER, Buffer.alloc(0)).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  getGocdeFile() {
    return this.send(0xac, 0x00, PeerId.CONTROLLER, Buffer.alloc(0)).then(
      ({ response, packet }) => {
  ...
  laserCalibration(calibrationMode: number) {
    ...
    return this.send(0xa8, 0x02, PeerId.CONTROLLER, info.toBuffer()).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  laserCalibrationSave(type: number) {
    return this.send(0xa8, 0x03, PeerId.CONTROLLER, Buffer.alloc(1, type)).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  SetLaserPower(key: number, power: number) {
    ...
    return this.send(0x12, 0x02, PeerId.CONTROLLER, info.toBuffer()).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  SetBrightness(key: number, brightness: number) {
    ...
    return this.send(0x12, 0x03, PeerId.CONTROLLER, tobuffer).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  SetFocalLength(key: number, focalLength: number) {
    ...
    return this.send(0x12, 0x04, PeerId.CONTROLLER, tobuffer).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  TemperatureProtect(
    ...
    return this.send(0x12, 0x05, PeerId.CONTROLLER, tobuffer).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  SetLaserLock(key: number, lockStatus: number) {
    ...
    return this.send(0x12, 0x07, PeerId.CONTROLLER, tobuffer).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  GetFDMInfo(key: number) {
    ...
    return this.send(0x10, 0x01, PeerId.CONTROLLER, info.toBuffer()).then(
      ({ response, packet }) => {
  ...
  GetHotBed(key: number) {
    ...
    return this.send(0x14, 0x01, PeerId.CONTROLLER, info.toBuffer()).then(
      ({ response, packet }) => {
  ...
  SetExtruderTemperature(
    ...
    return this.send(0x10, 0x02, PeerId.CONTROLLER, tobuffer).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  SetFilamentstatus(
    key: number,
    extruderIndex: number,
    filamentstatus: number
  ) {
    const tobuffer = Buffer.alloc(1 + 1 + 1, 0);
    writeUint8(tobuffer, 0, key);
    writeUint8(tobuffer, 1, extruderIndex);
    writeUint8(tobuffer, 2, filamentstatus);
    return this.send(0x10, 0x04, PeerId.CONTROLLER, tobuffer).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  SwitchExtruder(key: number, extruderIndex: number) {
    ...
    writeUint8(tobuffer, 0, key);
    writeUint8(tobuffer, 1, extruderIndex);
    return this.send(0x10, 0x05, PeerId.CONTROLLER, tobuffer).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  SetExtruderSpeed(key: number, fansIndex: number, speedLevel: number) {
    ...
    return this.send(0x10, 0x06, PeerId.CONTROLLER, tobuffer).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  SetExtruderOffset(
    ...
    return this.send(0x10, 0x07, PeerId.CONTROLLER, info.toBuffer()).then(
      ({ response, packet }) => {
        return { response, packet, data: {} };
      }
    );
  ...
  GetExtruderOffset(key: number) {
    return this.send(0x10, 0x08, PeerId.CONTROLLER, Buffer.alloc(1, key)).then(
      ({ response, packet }) => {
        const ExtruderOffsetInfo = new ExtruderOffset().fromBuffer(
          response.data
        );
  ...
  ExtruderMovement(
    ...
    const info = new ExtruderMovement(
      key,
      movementType,
      lengthIn,
      speedIn,
    ...
    return this.send(0x10, 0x09, PeerId.CONTROLLER, info.toBuffer()).then(
      ({ response, packet }) => {
  ...
  uploadFile(filePath: string) {
    ...
    this.setHandler(0xb0, 0x01, (data) => {
      ...
      // console.log('>>', index);
      this.log.info(
        `receive file chunk request: md5: ${md5HexStr}, index: ${index}`
      );
